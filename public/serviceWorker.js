const NEXTGPT_CACHE = "nextgpt-cache";
const NEXTGPT_FILE_CACHE = "nextgpt-file";
...
self.addEventListener("install", function (event) {
  ...
  event.waitUntil(
    caches.open(NEXTGPT_CACHE).then(function (cache) {
      return cache.addAll([]);
    }),
  );
});
...
async function upload(request, url) {
  ...
  const cache = await caches.open(NEXTGPT_FILE_CACHE)
